...
StaticCamera::StaticCamera(glm::vec3 position, glm::vec3 lookAtPoint,
                           glm::vec3 upVector)
    : Camera(),
      mPosition(position),
...
void StaticCamera::Update(float dt) { EventManager::EnableMouseCursor(); }
...
glm::vec3 StaticCamera::GetLookAt() const {
  return glm::normalize(mLookAtPoint - mPosition);
}