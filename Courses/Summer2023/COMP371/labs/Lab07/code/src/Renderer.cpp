...
void Renderer::Initialize() {
  ...
  // TODO 3.3 - Enable transparency
  // glEnable(GL_BLEND);
  // glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

  // Initialize the Quad particle texture
  Quad::initializeTexture();
...
void Renderer::Shutdown() {
  // Shaders
  for (vector<unsigned int>::iterator it = sShaderProgramID.begin();
       it < sShaderProgramID.end(); ++it) {
    glDeleteProgram(*it);
  }
...
//
GLuint Renderer::LoadShaders(std::string vertex_shader_path,
